...
from .exceptions import DataPacketError, DataPacketNotFoundError, DataPacketAlreadyExistsError, DataPacketValidationError, DataPacketPermissionError
...
class DataPacketAPI:
    ...
    def _setup_exception_handlers(self) -> None:
        ...
        async def handle_data_packet_already_exists(request, exc: DataPacketAlreadyExistsError):
            raise HTTPException(
        ...
        async def handle_data_packet_validation_error(request, exc: DataPacketValidationError):
            raise HTTPException(
        ...
        async def handle_data_packet_permission_error(request, exc: DataPacketPermissionError):
            raise HTTPException(
    ...
    async def create_data_packet(
        ...
        internal_data_packet = DataPacketInternal(**data_packet_create_request.data_packet.model_dump())
        ...
        created_data_packet = self.service.create_data_packet(
            data_packet_create_request.request_id,
            internal_data_packet
        )
    ...
    async def update_data_packet(
        ...
        """
        # Convert to internal model
        internal_data_packet = DataPacketInternal(**data_packet_update_request.data_packet.model_dump())

        # Pass data packet to service layer to be processed and updated
        # Service layer will handle all error cases and raise appropriate exceptions
        saved_data_packet = self.service.update_data_packet(
            data_packet_update_request.request_id,
            internal_data_packet
        )
