        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 204:
                    logger.info(f"✅ [{ENVIRONMENT}] Successfully sent metrics to VictoriaMetrics")
                else:
                    logger.warning(f"⚠️  [{ENVIRONMENT}] VictoriaMetrics responded with status {response.status_code}")
                    
        except httpx.ConnectError:
            logger.error(f"❌ [{ENVIRONMENT}] Failed to connect to VictoriaMetrics - is it running on {VM_URL}?")
        except Exception as e:
            logger.error(f"❌ [{ENVIRONMENT}] Error sending metrics to VictoriaMetrics: {e}")

@app.on_event("startup")
async def startup_event():
    """Start background tasks on startup."""
    logger.info(f"🚀 Starting FIDU Chat Lab ({ENVIRONMENT}) metrics service")
...
async def receive_metrics(request: Request):
    ...
    try:
        ...
        logger.info(f"📊 [{ENVIRONMENT}] Received {len(metrics)} metrics from frontend")
        ...
        for metric in metrics:
            ...
            try:
                ...
                logger.error(f"Error processing metric {metric_type}: {e}")
        
        return {"status": "success", "processed": len(metrics), "environment": ENVIRONMENT}
...
async def log_client_message(request: Request):
    ...
    try:
        ...
        logger.info(f"CLIENT [{ENVIRONMENT}]: {log_entry['message']}", extra={"client_data": log_entry['data']})
...
async def get_logs():
    ...
    return {"logs": client_logs[-50:], "environment": ENVIRONMENT}  # Return last 50 logs
