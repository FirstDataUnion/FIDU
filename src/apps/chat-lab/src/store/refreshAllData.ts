...
export async function refreshAllDataFromStorage(): Promise<void> {
  ...
  await Promise.allSettled([
    currentProfileId ? store.dispatch(fetchContexts(currentProfileId)) : Promise.resolve(),
    ...
    currentProfileId ? store.dispatch(fetchDocuments(currentProfileId)) : Promise.resolve(),

    currentProfileId ? store.dispatch(fetchSystemPrompts(currentProfileId)) : Promise.resolve(),
