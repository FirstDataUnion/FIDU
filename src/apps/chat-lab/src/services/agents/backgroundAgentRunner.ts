 ...
 */
function shouldDebounceEvaluation(
  ...
  if (lastEvaluation && now - lastEvaluation < DEBOUNCE_WINDOW_MS) {
    console.log(
      `🤖 [BackgroundAgents] Evaluation debounced for ${conversationId}:${messageId}:${turnCount} (evaluated ${now - lastEvaluation}ms ago)`
    );
    return true;
  }
...
function sliceMessagesForAgent(
  ...
  if (!messages || messages.length === 0) {
    console.warn(
      `🤖 [BackgroundAgents] Agent "${agent.name}" - No messages provided for evaluation`
    );
    return [];
  }
  ...
  switch (strategy) {
    case 'lastNMessages': {
      ...
      console.log(
        `🤖 [BackgroundAgents] Agent "${agent.name}" - Sliced ${messages.length} messages to last ${lastN} messages (got ${sliced.length} messages)`
      );
      return sliced;
    }
    case 'summarizeThenEvaluate':
      ...
      console.log(
        `🤖 [BackgroundAgents] Agent "${agent.name}" - Using all ${messages.length} messages (summarizeThenEvaluate strategy not yet implemented)`
      );
      return messages;
    case 'fullThreadIfSmall':
      ...
      console.log(
        `🤖 [BackgroundAgents] Agent "${agent.name}" - Using all ${messages.length} messages (fullThreadIfSmall strategy not yet implemented)`
      );
      return messages;
    default:
      // Unknown strategy, pass all messages
      console.warn(
        `🤖 [BackgroundAgents] Agent "${agent.name}" - Unknown context strategy "${strategy}", using all ${messages.length} messages`
      );
      return messages;
  }
}
...
function createAlertMetadata(
  ...
  if (result.response.actionType !== 'alert') {
    throw new Error(
      `🤖 [BackgroundAgents] createAlertMetadata: result.response.actionType is not 'alert': ${result.response.actionType}`
    );
  }
  return {
    agentId: agent.id,
 ...
 */
export async function maybeEvaluateBackgroundAgents(params: {
  profileId: string;
  conversationId: string;
  messages: ConversationSliceMessage[];
  ...
  if (!messageId) {
    console.warn(
      `🤖 [BackgroundAgents] messageId is required but was not provided. Alerts will not be persisted to messages.`
    );
  }
  ...
  if (shouldDebounceEvaluation(conversationId, messageId, turnCount)) {
    console.log(
      `🤖 [BackgroundAgents] Evaluation skipped - duplicate call debounced (conversationId: ${conversationId}, messageId: ${messageId}, turnCount: ${turnCount})`
    );
    return;
  }

  console.log(
    `🤖 [BackgroundAgents] Evaluation triggered - Turn: ${turnCount}, Conversation: ${conversationId}, Messages: ${messages.length}`
  );

  const storage = getUnifiedStorageService();
  const { backgroundAgents: customAgents } = await storage.getBackgroundAgents(
    undefined,
    1,
    100,
    profileId
  ...
  console.log(
    `🤖 [BackgroundAgents] Loaded preferences from localStorage:`,
    storedPrefs
  );

  const transformedBuiltInAgents =
    transformBuiltInAgentsWithPreferences(storedPrefs);
  ...
  const enabledAgents = allAgents.filter((a: any) => {
    if (!a.enabled) {
      console.log(
        `🤖 [BackgroundAgents] Agent "${a.name || a.id}" filtered out - disabled`
      );
      return false;
    }
    ...
    if (
      !a.actionType
      || (a.actionType !== 'alert' && a.actionType !== 'update_document')
    ) {
      console.warn(
        `🤖 [BackgroundAgents] Agent "${a.name || a.id}" is enabled but has invalid or missing actionType: "${a.actionType}". Skipping this agent - please fix the agent configuration.`
      );
  ...
  console.log(
    `🤖 [BackgroundAgents] Found ${transformedBuiltInAgents.length} built-in, ${customAgents?.length || 0} custom, ${allAgents.length} total agents, ${enabledAgents.length} enabled`
  );
  ...
  if (skippedAgents.length > 0) {
    console.warn(
      `🤖 [BackgroundAgents] ${skippedAgents.length} agent(s) skipped due to limit of ${MAX_ENABLED_AGENTS} enabled agents:`,
      skippedAgents.map((a: any) => a.name || a.id)
    );
  }

  console.log(
    `🤖 [BackgroundAgents] Evaluating ${agentsToEvaluate.length} agent(s):`,
    agentsToEvaluate.map((a: any) => ({
      id: a.id,
      name: a.name,
      runEveryNTurns: a.runEveryNTurns,
      actionType: a.actionType,
  ...
  const results = await Promise.allSettled(
    agentsToEvaluate.map(async (agent: any) => {
      ...
      console.log(
        `🤖 [BackgroundAgents] Agent "${agent.name}" (ID: ${agent.id}) - Checking cadence:`,
        {
      ...
      if (!meetsCadence) {
        console.log(
          `🤖 [BackgroundAgents] Agent "${agent.name}" skipped - not due to run (turn ${turnCount} % ${cadence} = ${turnCount % cadence} !== 0)`
        );
        return { skipped: true, reason: 'cadence', agent: agent.name };
      }

      console.log(
        `🤖 [BackgroundAgents] Agent "${agent.name}" - ✅ Meets cadence, starting evaluation...`
      );
      ...
      try {
        ...
        console.log(
          `🤖 [BackgroundAgents] Agent "${agent.name}" - Evaluation completed in ${evaluationTime}ms`
        );
        ...
        switch (result.response.actionType) {
          case 'alert': {
            const shouldNotify =
              result.response.notify
              && result.response.rating <= agent.verbosityThreshold;
            if (shouldNotify) {
              ...
              console.log(
                `🤖 [BackgroundAgents] Agent "${agent.name}" - ✅ Alert created successfully`
              );
              ...
              console.log(
                `🤖 [BackgroundAgents] Agent "${agent.name}" - ⚠️  Action type 'alert' but notify flag is false - skipping`
              );
              return null;
            }
          }
          case 'update_document': {
            if (!agent.outputDocumentId) {
              console.warn(
                `🤖 [BackgroundAgents] Agent "${agent.name}" - ⚠️  Action type 'update_document' requires outputDocumentId but it was not provided`
              );
              return null;
            }
            ...
            try {
              ...
              await storage.appendTextToDocument(
                agent.outputDocumentId,
                textToAppend,
                profileId
              );
              console.log(
                `🤖 [BackgroundAgents] Agent "${agent.name}" - ✅ Successfully appended text to document ${agent.outputDocumentId}`
              );
            } catch (error: any) {
              console.error(
                `🤖 [BackgroundAgents] Agent "${agent.name}" - ❌ Failed to append text to document:`,
                error?.message || error
              );
            }
          ...
          case 'none': {
            console.log(
              `🤖 [BackgroundAgents] Agent "${agent.name}" - ⚠️  Action type 'none' - skipping; reason: ${result.response.cancellationReason}`
            );
            return null;
          }
          default: {
            console.warn(
              `🤖 [BackgroundAgents] Agent "${agent.name}" - ⚠️  Unknown action type: ${(result.response as any).actionType}`
            );
            return null;
          }
        }
        ...
        console.error(
          `🤖 [BackgroundAgents] Agent "${agent.name}" - ❌ Evaluation failed:`,
          errorMessage,
          errorStatus ? `(Status: ${errorStatus})` : ''
        );
        console.error(
          `🤖 [BackgroundAgents] Agent "${agent.name}" - Error stack:`,
          e
        );
  ...
  const skipped = results.filter(
    r =>
      ...
      && ((r as PromiseFulfilledResult<any>).value === null
        || (r as PromiseFulfilledResult<any>).value === undefined)
  );

  console.log(
    `🤖 [BackgroundAgents] Evaluation complete - ${successes.length} succeeded, ${failures.length} failed, ${skipped.length} skipped`
  );

  if (failures.length > 0) {
    console.warn(
      `🤖 [BackgroundAgents] ${failures.length} background agent evaluation(s) failed, but chat flow continues normally`
    );
  }

  if (agentsToEvaluate.length === 0) {
    console.log(
      `🤖 [BackgroundAgents] No agents to evaluate (${enabledAgents.length} enabled, ${skippedAgents.length} skipped due to limit)`
    );
  }
  ...
  if (messageId && conversationId && conversationId !== 'current') {
    try {
      ...
      const alertsToPersist = results
        .filter(
          (
            ...
            agent: BackgroundAgent;
            result: any;
            alertMetadata: BackgroundAgentAlertMetadata;
          }> =>
            r.status === 'fulfilled'
            && r.value !== null
            && r.value !== undefined
            && 'alertMetadata' in r.value
        )
        .map(
          r =>
            (
      ...
      if (alertsToPersist.length > 0) {
        console.log(
          `🤖 [BackgroundAgents] Persisting ${alertsToPersist.length} alert(s) to message ${messageId} in conversation ${conversationId}`
        );
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
          try {
            ...
            console.log(
              `🤖 [BackgroundAgents] Attempt ${attempt + 1}/${maxRetries}: Loaded ${currentMessages.length} messages from storage`
            );
            ...
            if (messageIndex === -1) {
              ...
              console.warn(
                `🤖 [BackgroundAgents] Message ID ${messageId} not found in loaded messages. Available IDs: ${currentMessages.map(m => m.id).join(', ')}`
              );
            }
            ...
            if (attempt < maxRetries - 1) {
              console.log(
                `🤖 [BackgroundAgents] Message not found yet, waiting ${retryDelay}ms before retry...`
              );
              await new Promise(resolve => setTimeout(resolve, retryDelay));
            }
          } catch (error) {
            console.error(
              `🤖 [BackgroundAgents] Error loading messages on attempt ${attempt + 1}:`,
              error
            );
            if (attempt < maxRetries - 1) {
              await new Promise(resolve => setTimeout(resolve, retryDelay));
        ...
        if (messageIndex === -1 || !conversation) {
          assistantMessages = currentMessages.filter(
            (msg: Message) => msg.role === 'assistant'
          );
          console.warn(
            `🤖 [BackgroundAgents] Could not find target message for ${messageId} in conversation ${conversationId} after ${maxRetries} attempts. Alert metadata will not be persisted.`
          );
          console.warn(
            `🤖 [BackgroundAgents] Conversation found: ${!!conversation}, Available message IDs:`,
            currentMessages.map((m: Message) => m.id).slice(0, 10)
          );
          console.warn(
            `🤖 [BackgroundAgents] Available assistant messages:`,
            assistantMessages
              .map((m: Message) => ({ id: m.id, role: m.role }))
              .slice(0, 10)
          );
          console.warn(
            `🤖 [BackgroundAgents] Looking for message ID: ${messageId}`
          );
        } else {
          // Update message with alert metadata
          const targetMessage = currentMessages[messageIndex];
          console.log(
            `🤖 [BackgroundAgents] Found target message at index ${messageIndex}:`,
            {
              id: targetMessage.id,
              role: targetMessage.role,
              hasExistingAlerts:
                !!targetMessage.metadata?.backgroundAgentAlerts?.length,
              existingAlertCount:
                targetMessage.metadata?.backgroundAgentAlerts?.length || 0,
            }
          );
          ...
          const updatedMessages = currentMessages.map((msg, idx) => {
            if (idx === messageIndex) {
              ...
              console.log(
                `🤖 [BackgroundAgents] Updating message ${msg.id} with ${existingAlerts.length} existing + ${newAlerts.length} new alerts`
              );
          ...
          const persistedAlerts =
            verifyMessage?.metadata?.backgroundAgentAlerts || [];

          console.log(
            `🤖 [BackgroundAgents] ✅ Successfully persisted ${alertsToPersist.length} alert(s) to message metadata`
          );
          console.log(
            `🤖 [BackgroundAgents] Verification: Message ${verifyMessage?.id} now has ${persistedAlerts.length} total alert(s)`
          );
        }
      }
    } catch (error: any) {
      ...
      console.error(
        `🤖 [BackgroundAgents] Failed to persist alerts to message metadata:`,
        error?.message || error
      );
      console.error(
        `🤖 [BackgroundAgents] Alerts were still created and are available in alert history.`
      );
    }
  } else {
    if (!messageId) {
      console.warn(
        `🤖 [BackgroundAgents] No messageId provided - alerts will not be persisted to message metadata`
      );
    } else if (!conversationId) {
      console.warn(
        `🤖 [BackgroundAgents] No conversationId provided - alerts will not be persisted to message metadata`
      );
    }
  }
}
