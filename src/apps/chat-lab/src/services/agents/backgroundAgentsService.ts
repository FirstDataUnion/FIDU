  ...
  return {
    ...
    properties: {
      ...
      short_message: {
        type: 'string',
        description:
          'Brief notification message (1-2 sentences) suitable for toast notifications or popups. Keep it concise and actionable.',
      },
      description: {
        type: 'string',
        description:
          'Detailed explanation of the alert, including context, reasoning, and recommendations. This appears when users expand the notification for more details.',
      },
      severity: {
        type: 'string',
  ...
  return {
    ...
    properties: {
      heading: {
        type: 'string',
        description:
          'A very brief message suitable for a heading in a markdown document. Specific to this analysis of this conversation.',
      },
      content: {
        type: 'string',
        description:
          'The content of the response, which will be appended to a markdown document as a new section. This must be formatted as markdown and must not include any top-level headings (#). Unless the prompt says otherwise, aim for one or two paragraphs.',
      },
    },
    additionalProperties: false,
  };
...
function buildAlertAgentPrompt(
  ...
  const agentInstructions =
    ...
    || 'Analyze the conversation and provide an evaluation.';
...
function extractModelOutput(execStatus: any, agentName: string): string {
  ...
  console.log(
    `🤖 [BackgroundAgents] Agent "${agentName}" - Full execStatus structure:`,
    {
  ...
  if (content === undefined || content === null) {
    console.log(
      `🤖 [BackgroundAgents] Agent "${agentName}" - Primary path empty, trying alternatives...`
    );
    content =
      outputs.results?.[0]?.result
    ...
    if (content !== undefined && content !== null) {
      console.log(
        `🤖 [BackgroundAgents] Agent "${agentName}" - Found content via alternative path:`,
        typeof content,
        content?.toString().substring(0, 200)
      );
    } else {
      console.warn(
        `🤖 [BackgroundAgents] Agent "${agentName}" - No content found in any path. Full outputs:`,
  ...
  if (typeof content === 'string') {
    if (content.length === 0) {
      console.warn(
        `🤖 [BackgroundAgents] Agent "${agentName}" - Content is empty string`
      );
    }
    return content;
  }
  ...
  if (content === null || content === undefined) {
    console.warn(
      `🤖 [BackgroundAgents] Agent "${agentName}" - Model output is null/undefined. Full execStatus:`,
      JSON.stringify(execStatus, null, 2).substring(0, 1000)
    );
    return '{}';
  }
  ...
  try {
    ...
    if (!jsonString || jsonString === 'null' || jsonString === 'undefined') {
      console.warn(
        `🤖 [BackgroundAgents] Agent "${agentName}" - JSON stringify produced invalid result:`,
        jsonString
      );
      return '{}';
    }
    ...
    console.error(
      `🤖 [BackgroundAgents] Agent "${agentName}" - Error stringifying content:`,
      error,
      'Content:',
      content
    );
  ...
  try {
    ...
    if (start !== -1 && end > start) {
      try {
        ...
        console.warn(
          `🤖 [BackgroundAgents] Agent "${agentName}" - JSON repair failed`
        );
      }
    }
  }
...
function validateAndNormalizeResult(
  ...
  if (agent.actionType === 'alert') {
    return {
      ...validateAndNormalizeAlertResult(parsed, agent),
      actionType: agent.actionType,
    };
  }
  if (agent.actionType === 'update_document') {
    return {
      ...validateAndNormalizeUpdateDocumentResult(parsed, agent),
      actionType: agent.actionType,
    };
  }
  throw new Error(`Unsupported action type: ${agent.actionType}`);
...
function validateAndNormalizeAlertResult(
  ...
  if (!parsed || typeof parsed !== 'object') {
    console.warn(
      `🤖 [BackgroundAgents] Agent "${agent.name}" - Invalid parsed result, using defaults`
    );
    return {
      rating: 0,
      short_message: 'Evaluation failed - invalid response format',
      description:
        'The background agent evaluation could not be completed due to an invalid response format from the model.',
...
function validateAndNormalizeUpdateDocumentResult(
  ...
  return {
    heading: String(
      parsed.heading?.replace(/^#+\s*/, '').trim()
        || 'Background Agent Response: ' + agent.name
    ),
...
export async function evaluateBackgroundAgent(
  ...
  console.log(
    `🤖 [BackgroundAgents] Evaluating agent "${agent.name}" (ID: ${agent.id})`
  );
  console.log(
  ...
  try {
    ...
    if (
      ...
      console.warn(
        `🤖 [BackgroundAgents] Agent "${agent.name}" - Evaluation skipped due to authentication error`
      );
      return {
        agentId: agent.id,
        response: {
  ...
  let parseError:
    | { message: string; rawOutput: string; execStatus?: any }
    | undefined;
  if (!parsed || rawModelOutput.length === 0 || rawModelOutput === '{}') {
    ...
    console.error(
      `🤖 [BackgroundAgents] Agent "${agent.name}" - Parse error:`,
      errorMessage
    );
    console.error(
      `🤖 [BackgroundAgents] Agent "${agent.name}" - Full execStatus:`,
  ...
  if (normalized.actionType !== agent.actionType) {
    console.error(
      `🤖 [BackgroundAgents] Agent "${agent.name}" - Action type mismatch: ${normalized.actionType} !== ${agent.actionType}`
    );
    throw new Error(
      `Action type mismatch: ${normalized.actionType} !== ${agent.actionType}`
    );
  }
  ...
  console.log(
    `🤖 [BackgroundAgents] Agent "${agent.name}" - Evaluation result:`,
    {
