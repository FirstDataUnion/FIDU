...
describe('buildCompletePrompt', () => {
  const mockSystemPrompt = { content: 'You are a helpful assistant.', id: 'sp-1', name: 'Test' };
  ...
  describe('Context Handling', () => {
    it('should handle no contexts', () => {
      ...
      expect(result).not.toContain('Given the following existing background context');
    });

    it('should handle single context as array', () => {
      const contexts = [{ body: 'Context content', title: 'Context 1', id: 'ctx-1' }];
      const result = buildCompletePrompt([], [], contexts, [], 'Test prompt');
      
      expect(result).toContain('Given the following existing background context:');
      expect(result).toContain('Context content');
      expect(result).toContain('Answer the following prompt, keeping the existing context in mind:');
    });

    it('should handle single context as object (backward compatibility)', () => {
      const context = { body: 'Context content', title: 'Context 1', id: 'ctx-1' };
    ...
    it('should handle multiple contexts', () => {
      ...
      expect(result).toContain('Given the following existing background contexts:');
      ...
      expect(result).toContain('Answer the following prompt, keeping the existing contexts in mind:');
    });
    ...
    it('should handle context with conversation history - single context', () => {
      ...
      const messages: Message[] = [
        ...
        { ...mockMessage, content: 'AI response', role: 'assistant', id: 'msg-2' }
      ];
      const result = buildCompletePrompt([], [], contexts, messages, 'Test prompt');
      ...
      expect(result).toContain('keeping the existing context of the conversation in mind');
    });
    ...
    it('should handle context with conversation history - multiple contexts', () => {
      ...
      expect(result).toContain('Given the following existing background contexts:');
      expect(result).toContain('keeping the existing contexts of the conversation in mind');
    });
  });
