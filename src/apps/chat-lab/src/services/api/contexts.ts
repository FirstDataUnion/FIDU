...
const transformContextToDataPacket = (
  context: any,
  profileId: string
): ContextDataPacket => {
  return {
...
export const createContextsApi = () => {
  return {
    getAll: async (
      ...
      try {
        const response = await fiduVaultAPIClient.get<ContextDataPacket[]>(
          '/data-packets',
          {
            params: {
    ...
    createContext: async (context: any, profileId: string) => {
      try {
        ...
        const response = await fiduVaultAPIClient.post<ContextDataPacket>(
          '/data-packets',
    ...
    updateContext: async (context: any, profileId: string) => {
      try {
        ...
        const response = await fiduVaultAPIClient.put<ContextDataPacket>(
          '/data-packets/' + context.id,
