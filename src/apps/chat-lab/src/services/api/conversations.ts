...
const transformDataPacketToConversation = (
  packet: ConversationDataPacket
): Conversation => {
  ...
  if (packet.data.originalPrompt) {
    ...
    if (
      packet.data.originalPrompt?.systemPromptIds
      && packet.data.originalPrompt.systemPromptIds.length > 0
    ) {
      // New format with multiple system prompts
      systemPrompts = packet.data.originalPrompt.systemPromptIds.map(
        (id, index) => ({
          id: id,
          name:
            packet.data.originalPrompt!.systemPromptNames?.[index] || 'Unknown',
          content:
            packet.data.originalPrompt!.systemPromptContents?.[index] || '',
          description: '',
          tokenCount: 0,
          isDefault: false,
          isBuiltIn: true,
  ...
  const modelsUsed =
    packet.data.modelsUsed && Array.isArray(packet.data.modelsUsed)
  ...
  return {
    id: packet.id,
    title:
      packet.data.conversationTitle
      || packet.data.conversationUrl
      || 'Untitled Conversation',
    platform: (packet.data.sourceChatbot?.toLowerCase() || 'other') as
      | 'chatgpt'
      | 'claude'
      | 'gemini'
      | 'other',
...
const transformConversationToDataPacket = (
  profile_id: string,
  conversation: Partial<Conversation>,
  messages: Message[],
  originalPrompt?: Conversation['originalPrompt']
): ConversationDataPacket => {
  ...
  const mergedModelsUsed = [
    ...new Set([...existingModelsUsed, ...computedModelsUsed]),
  ];
  ...
  return {
    ...
    tags: [
      ...PROTECTED_TAGS,
      ...(conversation.tags?.filter(tag => !PROTECTED_TAGS.includes(tag as any))
        || []),
    ],
    data: {
      ...
      originalPrompt: originalPrompt
        ? {
            ...
            embellishmentIds:
              (originalPrompt as any).embellishments?.map((emb: any) => emb.id)
              || [],
...
const transformConversationToDataPacketUpdate = (
  conversation: Partial<Conversation>,
  messages: Message[],
  originalPrompt?: Conversation['originalPrompt']
): ConversationDataPacketUpdate => {
  if (!conversation.id) {
...
export const conversationsApi = {
  createConversation: async (
    ...
    const dataPacket = transformConversationToDataPacket(
      profile_id,
      conversation,
      messages,
      originalPrompt
    ...
    const response = await fiduVaultAPIClient.post<ConversationDataPacket>(
      '/data-packets',
  ...
  updateConversation: async (
    ...
    const dataPacket = transformConversationToDataPacketUpdate(
      conversation,
      messages,
      originalPrompt
    );
    ...
    const response = await fiduVaultAPIClient.put<ConversationDataPacket>(
      '/data-packets/' + conversation.id,
  ...
  getAll: async (
    ...
    try {
      const response = await fiduVaultAPIClient.get<ConversationDataPacket[]>(
        '/data-packets',
        {
          params: queryParams,
          paramsSerializer: {
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        ...
        const messages = packet.data.interactions.map((interaction, index) => ({
          ...
          platform:
            interaction.model || packet.data.sourceChatbot.toLowerCase(),
          metadata: {
            ...
            ...(interaction.metadata
              ? Object.fromEntries(
                  Object.entries(interaction.metadata).filter(
                    ([key]) =>
                      key !== 'attachments' && key !== 'backgroundAgentAlerts'
                  )
                )
              : {}),
          },
          attachments:
            interaction.attachments?.map((attachment, attIndex) => ({
        ...
        const messagesWithAlerts = messages.filter(
          m => (m.metadata?.backgroundAgentAlerts?.length ?? 0) > 0
        );
        if (messagesWithAlerts.length > 0) {
          const totalAlerts = messagesWithAlerts.reduce(
            (sum, m) => sum + (m.metadata?.backgroundAgentAlerts?.length ?? 0),
            0
          );
          console.log(
            `📋 [ConversationsAPI] Loaded conversation ${conversationId}: ${messages.length} messages, ${messagesWithAlerts.length} with alerts (${totalAlerts} total alerts)`
          );
          messagesWithAlerts.forEach(msg => {
            const alerts = msg.metadata?.backgroundAgentAlerts || [];
            console.log(
              `📋 [ConversationsAPI]   Message ${msg.id} (${msg.role}): ${alerts.length} alert(s)`,
              alerts.map((a: any) => ({
          ...
          console.log(
            `📋 [ConversationsAPI] Loaded conversation ${conversationId}: ${messages.length} messages, no alerts found`
          );
        }
        ...
        throw new Error(
          response.message || 'Failed to get conversation messages'
        );
      }
    } catch (error) {
