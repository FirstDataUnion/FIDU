...
export class NLPWorkbenchAPIClient {
   ...
   */
  private async enhanceRequestWithAPIKeys(
    input: string
  ): Promise<{ input: string; llm_api_keys?: Record<string, string> }> {
    const requestData: {
      input: string;
    ...
    try {
      ...
      for (const provider of providers) {
        ...
        if (apiKey) {
          ...
          console.log(
            `✅ [NLPWorkbench] Found API key for ${provider}, preview: ${keyPreview}`
          );
        }
      }
      ...
      if (Object.keys(llmApiKeys).length > 0) {
        ...
        console.log(
          `✅ [NLPWorkbench] Added llm_api_keys with ${Object.keys(llmApiKeys).length} provider(s):`,
          Object.keys(llmApiKeys)
        );
      } else {
        console.warn(
          `⚠️ [NLPWorkbench] No API keys available for any provider`
        );
      }
    } catch (error) {
    ...
    console.log(`📤 [NLPWorkbench] Request data structure:`, {
      ...
      llm_api_keys: requestData.llm_api_keys
        ? Object.keys(requestData.llm_api_keys)
        : 'none',
    });
   ...
   */
  async executeModelAgent(
    modelId: string,
    input: string
  ): Promise<NLPWorkbenchExecuteResponse> {
    ...
    console.log(
      `🚀 [NLPWorkbench] Executing model: ${modelConfig.name} (${actualModelId})`
    );
   ...
   */
  async executeGeminiFlashGeneralAgent(
    input: string
  ): Promise<NLPWorkbenchExecuteResponse> {
   ...
   */
  async executeGeminiProGeneralAgent(
    input: string
  ): Promise<NLPWorkbenchExecuteResponse> {
   ...
   */
  async executeClaudeHaikuGeneralAgent(
    input: string
  ): Promise<NLPWorkbenchExecuteResponse> {
   ...
   */
  async executeClaudeSonnetGeneralAgent(
    input: string
  ): Promise<NLPWorkbenchExecuteResponse> {
   ...
   */
  async executeClaudeOpus41GeneralAgent(
    input: string
  ): Promise<NLPWorkbenchExecuteResponse> {
   ...
   */
  async executeChatGPT35TurboGeneralAgent(
    input: string
  ): Promise<NLPWorkbenchExecuteResponse> {
   ...
   */
  async executeChatGPT40GeneralAgent(
    input: string
  ): Promise<NLPWorkbenchExecuteResponse> {
   ...
   */
  async executeChatGPT40TurboGeneralAgent(
    input: string
  ): Promise<NLPWorkbenchExecuteResponse> {
   ...
   */
  async executeChatGPT40MiniGeneralAgent(
    input: string
  ): Promise<NLPWorkbenchExecuteResponse> {
   ...
   */
  async executeChatGPT50GeneralAgent(
    input: string
  ): Promise<NLPWorkbenchExecuteResponse> {
   ...
   */
  async executeChatGPT50MiniGeneralAgent(
    input: string
  ): Promise<NLPWorkbenchExecuteResponse> {
   ...
   */
  async executeChatGPT50NanoGeneralAgent(
    input: string
  ): Promise<NLPWorkbenchExecuteResponse> {
  ...
  async executeAgentAndWait(
    ...
    while (Date.now() - startTime < maxWaitTime) {
      try {
        ...
        if (pollCount % 10 === 0) {
          console.log(
            `NLP Workbench polling update (${pollCount} polls, ${Math.round((Date.now() - startTime) / 1000)}s elapsed):`,
            {
              executionId,
    ...
    console.error(
      'NLP Workbench execution timed out - comprehensive debug info:',
      debugInfo
    );

    throw new ApiError(408, 'NLP Workbench execution timed out', debugInfo);
  }
}
...
export const createNLPWorkbenchAPIClientWithSettings = () =>
