...
describe('SyncService', () => {
  ...
  beforeEach(() => {
    ...
    mockDbManager = {
      importConversationsDB: jest
        .fn()
        .mockResolvedValue({ inserted: 0, updated: 0, forked: 0 }),
      importAPIKeysDB: jest.fn().mockResolvedValue(undefined),
      exportConversationsDB: jest
        .fn()
        .mockResolvedValue(new Uint8Array([1, 2, 3])),
      exportAPIKeysDB: jest.fn().mockResolvedValue(new Uint8Array([4, 5, 6])),
      getPendingChangesCount: jest
        .fn()
    ...
    mockDriveService = {
      ...
      checkFilesExist: jest.fn().mockResolvedValue({
        conversations: true,
        apiKeys: true,
        metadata: true,
      }),
      downloadConversationsDB: jest
        .fn()
        .mockResolvedValue(new Uint8Array([1, 2, 3])),
  ...
  describe('getCurrentUserName', () => {
    ...
    it('should fall back to Google user name', () => {
      mockAuthService.getCachedUser.mockReturnValue({
        name: 'Google User',
  ...
  describe('syncFromDrive - Personal Workspace', () => {
    it('should download both conversations and API keys for personal workspace', async () => {
      ...
      expect(mockDriveService.downloadConversationsDB).toHaveBeenCalledWith(
        '1'
      );
    ...
    it('should complete successfully even if refreshAllDataFromStorage fails', async () => {
      mockRefreshAllDataFromStorage.mockClear();
      mockRefreshAllDataFromStorage.mockRejectedValue(
        new Error('Mock: Failed to refresh Redux state')
      );
  ...
  describe('syncFromDrive - Shared Workspace', () => {
    ...
    it('should throw error if file ID download fails with non-404 error', async () => {
      ...
      await expect(service.syncFromDrive('1', fileIds)).rejects.toThrow(
        'Network error'
      );
    });
  ...
  describe('syncToDrive - Personal Workspace', () => {
    it('should upload both conversations and API keys for personal workspace', async () => {
      ...
      expect(mockDbManager.markDataPacketsAsSynced).toHaveBeenCalledWith([
        'dp-1',
      ]);
  ...
  describe('syncToDrive - Shared Workspace', () => {
    ...
    it('should pass username to importConversationsDB during merge for shared workspace', async () => {
      localStorage.setItem(
        'user',
        JSON.stringify({ email: 'bob@company.com' })
      );
      mockDbManager.getPendingChangesCount.mockResolvedValue({
        dataPackets: 1,
        apiKeys: 0,
  ...
  describe('fullSync', () => {
    it('should call syncFromDrive then syncToDrive', async () => {
      ...
      expect(
        callOrder.filter(c => c === 'checkFiles').length
    ...
    it('should store last sync time after successful sync', async () => {
      ...
      expect(new Date(storedTime!).getTime()).toBeGreaterThan(
        Date.now() - 1000
      );
    });
    ...
    it('should prevent concurrent syncs', async () => {
      ...
      const checkFilesPromise = new Promise<void>(resolve => {
        resolveCheckFiles = resolve;
      });
    ...
    it('should pass fileIds to syncFromDrive for shared workspaces', async () => {
      ...
      const fileIds = {
        conversationsDbId: 'conv-id',
        metadataJsonId: 'meta-id',
      };
  ...
  describe('Error Handling', () => {
    it('should continue upload if download fails during syncToDrive', async () => {
      ...
      mockDbManager.getPendingDataPackets.mockResolvedValue([{ id: 'dp-1' }]);
      mockDriveService.downloadConversationsDB.mockRejectedValue(
    ...
    it('should set error state on sync failure', async () => {
      mockDriveService.checkFilesExist.mockRejectedValue(
        new Error('Network error')
      );
      mockDbManager.getPendingChangesCount.mockResolvedValue({
        dataPackets: 0,
      ...
      await expect(service.fullSync({ forceUpload: true })).rejects.toThrow(
        'Network error'
      );
