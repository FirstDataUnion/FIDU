...
describe('FiduAuthService Auth Interceptor', () => {
  ...
  beforeEach(async () => {
    ...
    fiduAuthService.setTokens(initialAccessToken, 'refresh_token', {
      id: '1',
    ...
    // Set up interceptors
    axiosClient.interceptors.request.use(authInterceptor.request);
  ...
  describe('401 response with successful token refresh and retry', () => {
    it('should refresh token on 401, retry request, and return 200 response', async () => {
      ...
      fetchMock.mockImplementation(
        (input: string | URL | globalThis.Request, _init?: RequestInit) => {
  ...
  describe('401 response with 401 refresh token response', () => {
      ...
      fetchMock.mockImplementation(
        (input: string | URL | globalThis.Request, _init?: RequestInit) => {
          ...
          return Promise.resolve({
            ...
            status: 401,
            json: () =>
              Promise.resolve({ detail: 'Refresh token expired or invalid' }),
          });
        }
      );
    });
    ...
    it('should clear tokens and dispatch logout on refresh failure', async () => {
      const clearAllAuthTokensSpy = jest.spyOn(
        fiduAuthService,
        'clearAllAuthTokens'
      );
