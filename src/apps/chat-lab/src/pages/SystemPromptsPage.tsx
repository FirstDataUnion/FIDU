    ...
    return (
      <Card
        ...
        sx={{
          ...
          position: 'relative',
          cursor:
            isSelectionMode && !systemPrompt.isBuiltIn ? 'pointer' : 'default',
        ...
        {systemPrompt.source && (
          <Box
            sx={{
              ...
              backgroundColor:
                systemPrompt.source === 'fabric'
                  ? 'rgba(25, 118, 210, 0.8)'
                  : systemPrompt.source === 'wharton'
                    ? 'rgba(76, 175, 80, 0.8)'
                    : systemPrompt.source === 'built-in'
                      ? 'rgba(0, 0, 0, 0.6)'
                      : 'rgba(156, 39, 176, 0.8)',
              color: 'white',
              px: 1,
...
const SystemPromptsPage = React.memo(() => {
  ...
  const currentTabPrompts = useMemo(() => {
    switch (activeTab) {
      case 1:
        return fabricPrompts;
      case 2:
        return whartonPrompts;
      case 3:
        return builtInPrompts;
      case 4:
        return userPrompts;
      default:
        return systemPrompts; // All
    }
  }, [
    activeTab,
    fabricPrompts,
  ...
  const filteredPrompts = useMemo(() => {
    ...
    if (selectedCategories.length > 0) {
      filtered = filtered.filter(
        prompt =>
          prompt.categories
          && prompt.categories.some(cat => selectedCategories.includes(cat))
      );
    }
  ...
  const handleEditSystemPrompt = useCallback(() => {
    if (selectedSystemPrompt) {
      setSystemPromptForm({
        ...
        categories: selectedSystemPrompt.categories
          ? selectedSystemPrompt.categories.join(', ')
          : '',
      });
  ...
  const handleTryPrompt = useCallback(
    (systemPrompt: any) => {
      ...
      const hasExistingState =
        existingMessages || existingContext || existingSystemPrompts;
          ...
          >
            ...
            >
              ...
              >
                ...
                <Box sx={{ display: { xs: 'inline', sm: 'none' } }}>Help</Box>
              </Link>
            </Box>
            <Typography
              variant="body1"
            ...
            >
              Manage system prompts that define AI behavior and personality for
              your conversations
            </Typography>
            <Typography
              variant="body2"
            ...
            >
              Our library of system prompts is a curated collection sourced from
              Open Source prompt libraries. Thanks and credit goes to the
              respective projects:
            </Typography>
            <Box component="ul" sx={{ mt: 1, pl: 2, mb: 0 }}>
              ...
              >
                <Link
                  href="https://gail.wharton.upenn.edu/prompt-library"
                  target="_blank"
                  rel="noopener noreferrer"
                  sx={{ color: 'primary.main', textDecoration: 'none' }}
                >
                ...
                - Ethan Mollick and Lilach Mollick (Creative Commons Attribution
                4.0 International)
              </Typography>
            </Box>
          </Box>
        ...
        {loading ? (
          ...
          >
            ...
            <Typography variant="body1" sx={{ opacity: 0.5 }}>
              No Custom System Prompts Available. Please configure storage to
              use this feature
            </Typography>
          </Box>
