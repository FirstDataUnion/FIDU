...
export default function PromptLabPage() {
  ...
  const fetchSavedPrompts = useCallback(async () => {
    ...
    try {
      const queryParams: DataPacketQueryParams = {
        tags: ["FIDU-CHAT-LAB-Prompt", "Saved-Prompt"],
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      // Create conversation in FIDU Vault backed
  ...
  const fetchExecutionHistory = useCallback(async () => {
    ...
    try {
      const queryParams: DataPacketQueryParams = {
        tags: ["FIDU-CHAT-LAB-Prompt", "Prompt-History"],
        profile_id: currentProfile.id,
        limit: 100,
        offset: 0,
        sort_order: "desc"
  ...
  const handleSendFollowUp = useCallback(async (message: string) => {
    ...
    try {
      ...
      // Update conversation in FIDU Vault backed
    ...
    <Box sx={{ p: 3, pr: stackMinimized ? 3 : 45 }}>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="body1" color="text.secondary">
          design, construct, optimise, and send your AI prompts to a range of models
        </Typography>
      </Box>
          ...
          <Card>
            <CardContent>
              <Typography variant="h6" sx={{ mb: 2 }}>
                Context Suggestions [Work in Progress!]
              </Typography>
