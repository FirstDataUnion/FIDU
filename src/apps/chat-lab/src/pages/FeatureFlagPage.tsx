...
export default function FeatureFlagPage(): React.JSX.Element {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, p: 5 }}>
      <Typography
        variant="h4"
        sx={{ fontWeight: 600, fontSize: { xs: '1.75rem', sm: '2.125rem' } }}
      >
        <ExperimentalIcon
          sx={{
            fontSize: '2rem',
      ...
      >
        This page lets you change which features are enabled throughout the rest
        of the ChatLab. However, the changes only affect the UI (the way you see
        the app), not the actual functionality. For example, if you enable a
        Background Agent to write to a Document and then turn off the Background
        Agent and Document features, the agent will still run on each prompt and
        write to the document, but you won't be able to see either of them!
        Similarly, if you choose a specific model and then turn off the Model
        Selection feature, the model will still be used, but you won't be able
        to change it.
      </Typography>
      <Typography
        variant="h5"
      ...
      >
        Optional features are not required for the core functionality of the
        app, but can be enabled to help you make the most of FIDU Chat Lab. The
      ...
      <ul
        style={{ listStyleType: 'none', paddingLeft: '10px', marginTop: '0' }}
      >
        <li>
        ...
        <li>
          <DefaultIcon sx={{ fontSize: '1.2rem', verticalAlign: 'middle' }} />{' '}
