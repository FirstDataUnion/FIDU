...
export default function ContextsPage() {
  ...
  const handleUpdateContextSubmit = useCallback(async () => {
    if (!currentProfile?.id || !selectedContext || !contextForm.title.trim())
      return;
  ...
  const handleAddConversationToContext = useCallback(
    async (conversation: Conversation) => {
      ...
      try {
        ...
        const conversationContent = messagesResult
  ...
  const handleAddConversationToNewContext = useCallback(
    async (conversation: Conversation) => {
  ...
  const isDirectoryAccessError =
    error
    && (error.includes('No directory access')
      || error.includes('Please select a directory first'));
            ...
            >
              <Box>
                ...
                <Typography variant="body1" color="text.secondary">
                  Manage your conversation contexts, references, and knowledge
                  bases
                </Typography>
              </Box>
