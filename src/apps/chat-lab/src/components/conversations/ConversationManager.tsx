...
export const ConversationManager = React.memo<ConversationManagerProps>(({
  conversations,
  onCloseConversation,
  ...
  const activeConversation = useMemo(() => 
    conversations.find(tab => tab.isActive && !tab.isMinimized), 
    [conversations]
  );
  
  const minimizedTabs = useMemo(() => 
    conversations.filter(tab => tab.isMinimized), 
    [conversations]
  );
  ...
  }, [expandedTab, onActivateConversation]);

  const handleMinimize = useCallback((e: React.MouseEvent, tabId: string) => {
    e.stopPropagation();
  ...
  // Memoize minimized tab rendering to prevent unnecessary re-renders
  const minimizedTabsContent = useMemo(() => (
  ...
  ), [minimizedTabs, handleTabClick, handleMinimize, handleClose, hoveredTab]);

  // Memoize active conversation window to prevent unnecessary re-renders
  ...
  }, [activeConversation, onCloseConversation, onSendMessage, onMinimizeConversation, isSendingFollowUp, error]);
