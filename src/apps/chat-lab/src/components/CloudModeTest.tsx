...
import { PlayArrow, Refresh, Schedule, CloudSync } from '@mui/icons-material';
...
const CloudModeTest: React.FC = () => {
  ...
  const createTestData = async () => {
    ...
    try {
      ...
      setTestData({
        message:
          'Test conversation created! This should trigger auto-sync in 5 minutes.',
      ...
      <Stack spacing={3}>
              ...
              <Stack spacing={2}>
                <Box>
                  ...
                  <Stack direction="row" spacing={1} flexWrap="wrap">
                    <Chip
                      icon={<CloudSync />}
                      label={
                        syncStatus.smartAutoSync.enabled
                          ? 'Enabled'
                          : 'Disabled'
                      }
                      color={
                        syncStatus.smartAutoSync.enabled ? 'success' : 'default'
                      }
                      size="small"
                    />
                    <Chip
                      ...
                      color={
                        syncStatus.smartAutoSync.nextSyncScheduled
                          ? 'primary'
                          : 'default'
                      }
                      size="small"
                    ...
                    <Chip
                      ...
                      color={
                        syncStatus.smartAutoSync.hasUnsyncedData
                          ? 'warning'
                          : 'success'
                      }
                      size="small"
                ...
                <Box>
                  ...
                  <Typography variant="body2" component="div">
                    ...
                    {formatTime(syncStatus.smartAutoSync.lastActivity)}
                    <br />
                    <strong>Last Sync Attempt:</strong>{' '}
                    {formatTime(syncStatus.smartAutoSync.lastSyncAttempt)}
                    <br />
                    <strong>Retry Count:</strong>{' '}
                    {syncStatus.smartAutoSync.retryCount}
                  </Typography>
                </Box>
              </Stack>
            </CardContent>
          ...
          <Card>
            <CardContent>
              ...
              <Typography variant="body2" component="div">
                ...
                <br />
                <strong>Error:</strong> {syncStatus.error || 'None'}
                <br />
                <strong>Auto-Sync Enabled:</strong>{' '}
                {syncStatus.autoSyncEnabled ? 'Yes' : 'No'}
              </Typography>
            </CardContent>
          </Card>
        )}
        ...
        <Card>
          <CardContent>
            ...
            <Typography variant="body2" component="div">
              1. <strong>Create Test Data:</strong> Click "Create Test Data" to
              add a test conversation
              <br />
              2. <strong>Watch Countdown:</strong> See countdown timer in header
              (no activity timeout)
              <br />
              3. <strong>Auto-Sync:</strong> Sync triggers when countdown
              reaches zero
              <br />
              4. <strong>Check Status:</strong> Use "Refresh Status" to see
              current state
              <br />
              5. <strong>Force Sync:</strong> Use "Force Sync" to bypass
              countdown timing
            </Typography>
          </CardContent>
        </Card>
