...
function shouldTreatAsBulletPoint(text: string, hyphenIndex: number): boolean {
  const beforeHyphen = text.substring(
    Math.max(0, hyphenIndex - 10),
    hyphenIndex
  );
  const afterHyphen = text.substring(
    hyphenIndex + 1,
    Math.min(text.length, hyphenIndex + 20)
  );

  // Check if it's at the start of a line (after whitespace/newline)
  const isAtLineStart =
    /^\s*$/.test(beforeHyphen) || beforeHyphen.endsWith('\n');
  ...
  const isCompoundWord = /[a-zA-Z0-9]-[a-zA-Z0-9]/.test(
    text.substring(hyphenIndex - 1, hyphenIndex + 2)
  );
  ...
  const isDateRange = /[0-9]{4}-[0-9]{4}/.test(
    text.substring(hyphenIndex - 4, hyphenIndex + 5)
  );
...
export function validateMarkdownContent(content: string): {
  ...
  if (hyphenMatches && hyphenMatches.length > 0) {
    ...
    if (problematicHyphens > 0) {
      issues.push(
        `Found ${problematicHyphens} hyphens that might be incorrectly rendered as bullet points`
      );
      suggestions.push(
        'Consider using the preprocessMarkdown function to fix formatting issues'
      );
    }
  }
