...
export function analyzeRequestDuration(
  ...
  const hasVeryLargeContext =
    contextLength > 5000 || conversationLength > 10000;
  ...
  if (hasLargeContext) {
    reasons.push(
      `Large context (${(contextLength + conversationLength).toLocaleString()} characters)`
    );
  }
...
export function getLongRequestMessage(analysis: LongRequestAnalysis): string {
  ...
  const reasonsText =
    analysis.reasons.length > 1
      ? analysis.reasons.slice(0, -1).join(', ')
        + ', and '
        + analysis.reasons[analysis.reasons.length - 1]
      : analysis.reasons[0];
 ...
 */
export function shouldShowLongRequestWarning(
