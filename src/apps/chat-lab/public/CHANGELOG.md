...
### Changed
- **Google Drive refresh tokens**: Google Drive refresh tokens are now stored in the FIDU identity service provider token vault instead of browser HTTP-only cookies, so the connection works across devices and aligns with server-side token storage practice. Returning users may need to reconnect Google Drive once after upgrading.
...
- **Page Optimisation**: The whole chatlab app should feel a lot snappier! 
...
- **Add message to context**: The 'copy' button on message bubbles also allow you to add messages straight to a new or existing context. 
...
- **Streaming Responses**: ChatLab now support streamed responses from models, allowing the response to start appearing on screen much faster than before. 

- **ZDR Filtering**: ChatLab now filters all models to provide only models with ZDR (Zero Data Retention) complient policies, meaning that no data is ever stored by them or their providers.  

- **Image Generation Support**: Chatlab now supports the display and saving of images generated by models. These are saved to your Google Drive along with your conversation. Note that images are not currently being encrypted. NOTE: while image generation works in shared workspaces, the sharing of the images is not yet supported. You will not be able to see inline images generated by other workspace members, but they will be accesable via the Google Drive UI.  

### Fixed
- **AutoScroll**: Improved auto-scroll behaviour in the Chat page to better auto-scroll to the start of model responses in a more consistent fashion. 
