...
class TestExceptionHandlers:
    ...
    @patch("fidu_vault.data_packets.api.get_user_from_identity_service")
...
class TestCreateDataPacket:
    ...
    @patch("fidu_vault.data_packets.api.get_user_from_identity_service")
    def test_raises_401_if_identity_service_returns_unauthorized(
        self,
        mock_get_user,
        api,
        test_client,
        ...
        """Test that create data packet raises a 401 if identity service returns unauthorized."""
        # Mock the identity service to raise HTTPException (unauthorized)
    ...
    @patch("fidu_vault.data_packets.api.get_user_from_identity_service")
    def test_raises_403_if_identity_service_returns_forbidden(
        self,
        mock_get_user,
        api,
        test_client,
        ...
        """Test that create data packet raises a 403 if identity service returns forbidden."""
        # Mock the identity service to raise HTTPException (forbidden)
        mock_get_user.side_effect = HTTPException(
    ...
    @patch("fidu_vault.data_packets.api.get_user_from_identity_service")
    def test_raises_503_if_identity_service_unavailable(
        self,
        mock_get_user,
        api,
        test_client,
        ...
        """Test that create data packet raises a 503 if identity service is unavailable."""
        # Mock the identity service to raise HTTPException (service unavailable)
        mock_get_user.side_effect = HTTPException(
            status_code=503, detail="Identity service is currently unavailable"
        )
        ...
        assert response.json()["detail"] == "Identity service is currently unavailable"
    ...
    def test_raises_401_if_no_authorization_header(
        ...
        """Test that create data packet raises a 401 if no authorization header is provided."""
        # Mock the identity service to raise HTTPException (no token)
        mock_get_user.side_effect = HTTPException(
        ...
        # Call create data packet without authorization header
        json_compatible_request = jsonable_encoder(request)
        response = test_client.post(
            "/api/v1/data-packets",
            json=json_compatible_request,
...
class TestUpdateDataPacket:
    ...
    @patch("fidu_vault.data_packets.api.get_user_from_identity_service")
    def test_raises_401_if_identity_service_returns_unauthorized_update(
        self,
        mock_get_user,
        api,
        test_client,
        ...
        """Test that update data packet raises a 401 if identity service returns unauthorized."""
        # Mock the identity service to raise HTTPException (unauthorized)
        mock_get_user.side_effect = HTTPException(
            status_code=401, detail="Invalid or expired token"
        )
    ...
    @patch("fidu_vault.data_packets.api.get_user_from_identity_service")
    def test_raises_401_if_no_authorization_header_update(
        self,
        mock_get_user,
        api,
        test_client,
        ...
        """Test that update data packet raises a 401 if no authorization header is provided."""
        # Mock the identity service to raise HTTPException (no token)
        mock_get_user.side_effect = HTTPException(
            status_code=401, detail="Authorization token is required"
        )
        ...
        # Call update data packet without authorization header
        json_compatible_request = jsonable_encoder(request)
        response = test_client.put(
            "/api/v1/data-packets/test_packet_123",
            json=json_compatible_request,
...
class TestDeleteDataPacket:
    ...
    @patch("fidu_vault.data_packets.api.get_user_from_identity_service")
    def test_raises_401_if_identity_service_returns_unauthorized_delete(
        self,
        mock_get_user,
        api,
        test_client,
        ...
        """Test that delete data packet raises a 401 if identity service returns unauthorized."""
        # Mock the identity service to raise HTTPException (unauthorized)
        mock_get_user.side_effect = HTTPException(
            status_code=401, detail="Invalid or expired token"
        )
    ...
    @patch("fidu_vault.data_packets.api.get_user_from_identity_service")
    def test_raises_401_if_no_authorization_header_delete(
        self,
        mock_get_user,
        api,
        test_client,
        ...
        """Test that delete data packet raises a 401 if no authorization header is provided."""
        # Mock the identity service to raise HTTPException (no token)
        mock_get_user.side_effect = HTTPException(
            status_code=401, detail="Authorization token is required"
        )

        # Call delete data packet without authorization header
        response = test_client.delete("/api/v1/data-packets/test_packet_123")
...
class TestGetDataPacket:
    ...
    @patch("fidu_vault.data_packets.api.get_user_from_identity_service")
    def test_raises_401_if_identity_service_returns_unauthorized_get(
        self,
        mock_get_user,
        api,
        test_client,
        ...
        """Test that get data packet raises a 401 if identity service returns unauthorized."""
        # Mock the identity service to raise HTTPException (unauthorized)
        mock_get_user.side_effect = HTTPException(
            status_code=401, detail="Invalid or expired token"
        )
    ...
    @patch("fidu_vault.data_packets.api.get_user_from_identity_service")
    def test_raises_401_if_no_authorization_header_get(
        self,
        mock_get_user,
        api,
        test_client,
        ...
        """Test that get data packet raises a 401 if no authorization header is provided."""
        # Mock the identity service to raise HTTPException (no token)
        mock_get_user.side_effect = HTTPException(
            status_code=401, detail="Authorization token is required"
        )

        # Call get data packet without authorization header
        response = test_client.get("/api/v1/data-packets/test_packet_123")
...
class TestListDataPackets:
    ...
    @patch("fidu_vault.data_packets.api.get_user_from_identity_service")
    def test_raises_401_if_identity_service_returns_unauthorized_list(
        self,
        mock_get_user,
        api,
        test_client,
        ...
        """Test that list data packets raises a 401 if identity service returns unauthorized."""
        # Mock the identity service to raise HTTPException (unauthorized)
        mock_get_user.side_effect = HTTPException(
            status_code=401, detail="Invalid or expired token"
        )
    ...
    @patch("fidu_vault.data_packets.api.get_user_from_identity_service")
    def test_raises_401_if_no_authorization_header_list(
        self,
        mock_get_user,
        api,
        test_client,
        ...
        """Test that list data packets raises a 401 if no authorization header is provided."""
        # Mock the identity service to raise HTTPException (no token)
        mock_get_user.side_effect = HTTPException(
            status_code=401, detail="Authorization token is required"
        )

        # Call list data packets without authorization header
